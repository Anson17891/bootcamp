public class DemoNestedClass {
 private int x;

public DemoNestedClass(int x){
  this.x=x;
}

public DemoNestedClass(){}

//!Local Inner Class
//write a class inside a method
//the class can only be used in the method
public int sum(int x, int y){

  class Person{
    private int age;
    public Person(int age){
      this.age = age;
    }
    public int getAge(){
      return this.age;
    }
  }
  Person p1 = new Person(x + y);
  return p1.getAge();
  
}

//!Inner class, less common
  public class Dog{
    private int age;
    public Dog(int age){
      this.age = age;
    }
  
  public void bark(){
    System.out.println(this.age + ", " + x); //!Inner class can read outer class's attribute
  }
}

  //!Static class
  public static class Cat{ //!Class in a class ->Nested Class
    private String name;
    public Cat(String name){
      this.name = name;
    }
  }
  public static void main(String[] args) {
    //Static Nested Class
    new DemoNestedClass.Cat("John"); //if cat is builded outside this class, has to include "DemoNestedClass.""
  

    //Inner Class
    new DemoNestedClass().new Dog(10); //! inner class has to depend on outer class to be built
    //new Dog(10); //error
    DemoNestedClass dnc = new DemoNestedClass();
    dnc.new Dog(10);

    new DemoNestedClass(500).new Dog(40).bark();//"40, 500"
}

}
